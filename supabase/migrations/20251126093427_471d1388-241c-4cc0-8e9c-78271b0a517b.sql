-- 修改 get_dynamic_score_distribution 函数，将百分比转换为实际分数线
CREATE OR REPLACE FUNCTION public.get_dynamic_score_distribution(p_assessment_id bigint, p_subject_id bigint)
 RETURNS TABLE(group_name text, group_type text, score_range text, min_score integer, max_score integer, student_count bigint, percentage numeric, group_subtotal bigint, group_subtotal_percentage numeric)
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  WITH score_segments AS (
    ...
    SELECT 
      ...
      CAST(FLOOR(score_value) AS INT) as seg_min_score,
      CAST(FLOOR(score_value) AS INT) as seg_max_score,
    ...
    SELECT 
      ...
      CAST(FLOOR(score_value / 5) * 5 AS INT) as seg_min_score,
      CAST(FLOOR(score_value / 5) * 5 + 4 AS INT) as seg_max_score,
      COUNT(*) as seg_student_count
    FROM individual_scores
    WHERE assessment_id = p_assessment_id 
      AND subject_id = p_subject_id
