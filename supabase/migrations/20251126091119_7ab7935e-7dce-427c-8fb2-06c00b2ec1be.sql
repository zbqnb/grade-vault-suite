-- 修复 get_dynamic_score_distribution 函数中的列引用歧义问题
DROP FUNCTION IF EXISTS get_dynamic_score_distribution(bigint, bigint);
...
BEGIN
  ...
  WITH score_segments AS (
    ...
    SELECT 
      ...
      0 as seg_min_score,
      v_poor_threshold - 1 as seg_max_score,
      COUNT(*) as seg_student_count
    FROM individual_scores
    WHERE assessment_id = p_assessment_id 
      AND subject_id = p_subject_id
  ...
  group_totals AS (
    SELECT 
      ...
      SUM(seg_student_count) as gt_subtotal,
